...
class Cae(nn.Module):
    def __init__(self, word_embedder, categories, polarities):
        ...
        self.dropout_after_embedding = nn.Dropout(0.2)
        self.dropout_after_lstm = nn.Dropout(0.2)
...
model1.load_state_dict(torch.load('D:\code\deploy gradio\deploy-with-gradio\CAE_woshare_checkpoint20.pth',map_location=torch.device('cpu')))
model1.eval()

def infer_cae_wo_share(comment):
    model1.eval()
    # Tokenize input text
    ...
    return res
# print(infer_cae_wo_share('pin tot'))